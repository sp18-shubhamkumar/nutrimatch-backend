...
class FoodItemView(APIView):
    ...
    def post(self, rid, request):
        ...
        serializer = FoodItemSerializer(data=request.data, context={'restaurant':restaurant})
    ...
    def patch(self, request, rid, fid):
        ...
        try:
            ...
            return Response({'error': 'Food Item not found'}, status=404)
        serializer = FoodItemSerializer(food, data=request.data, partial=True, context={'restaurant':restaurant})
        if serializer.is_valid():
            serializer.save()
